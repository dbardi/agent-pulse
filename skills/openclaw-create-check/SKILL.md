...
description: Create new check scripts for the Agent Pulse heartbeat framework. Use when a user asks to create a new sensor, check, or monitor for Agent Pulse. Triggers on phrases like "create a pulse check", "add a new sensor", "write a check script", "monitor X with pulse", or "I want pulse to watch X". Also use when editing or improving an existing check script.
---
...
Every check script returns exactly one JSON object to stdout:
...
| `context` | `string` | What to tell the agent (include paths, values, counts) |
...
4. **10-second budget** — scripts are killed after 10 seconds
5. **Namespaced state** — the runner wraps your state_update under the check ID. Return flat data only
6. **Seen vs Processed** — for items needing agent action, track as `"seen"`. Runner updates to `"processed"` after handling. Only trigger on `"seen"` items
...
2. Create the check script in the `checks/` directory
